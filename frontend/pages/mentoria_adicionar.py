...
def mentoria_adicionar_page(tier: str = "mentoria_basico") -> None:
    design_tokens()
    if not is_logged_in():
    ...
    with ui.column().classes("w-full min-h-screen bg-[#F5F0E6] font-body items-center py-8 px-4"):
        ...
        with ui.column().classes("w-full items-center py-4 px-4"):
            with ui.card().classes(
                ...
                ui.label("Contratar Mentoria").classes(
                ...
                def handle_submit():
                    ...
                    result, error = adicionar_servico_mentoria(token, payload)
                    if result:
