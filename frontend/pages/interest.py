    ...
    with ui.column().classes("w-full min-h-screen bg-[#F5F0E6] font-body"):
        ...
        with ui.column().classes("w-full items-center justify-center flex-1 px-4 py-16"):
            with ui.card().classes(
                ...
                ui.icon("mail", size="4rem").classes("text-[#A6402F] mb-4")
                ui.label("Recebemos seu pedido!").classes(
                ...
                ).classes("text-[#4B5563] mb-6")

                ui.button(
                    "Voltar ao início",
                    on_click=lambda: ui.navigate.to("/")
                ).classes(
                    "mt-2 bg-[#A6402F] text-[#F5F0E6] rounded-none px-6 py-2 "
